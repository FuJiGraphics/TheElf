...
namespace Assets.PixelFantasy.Common.Scripts
{
    ...
    public class EffectManager : ScriptableObject
    {
        ...
        public SpriteEffect CreateSpriteEffect(GameObject go, string clipName, Vector3 scale)
        {
            var instance = Instantiate(SpriteEffectPrefab, go.transform.position, Quaternion.identity, null);

            instance.name = clipName;
            instance.transform.position = go.transform.position;
            instance.transform.localScale = scale;
            instance.GetComponent<SpriteRenderer>().sortingOrder = 50;
            instance.Play(clipName, Math.Sign(go.transform.localScale.x));
